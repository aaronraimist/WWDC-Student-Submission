...
- (void)stopTimer { // Slide Animation Timer
...
- (void)renderSlideToUnlockUIComponents { // Lower Compartment of Slider
    
	UIImage *trackImage = [UIImage imageNamed:@"slider1.png"];
    ...
    slideToUnlock_Background.alpha = 1;
	...
	[slideToUnlock_Slider setMinimumTrackImage:[UIImage imageNamed:@"slider3.png"] forState:UIControlStateNormal];
	[slideToUnlock_Slider setMaximumTrackImage:[UIImage imageNamed:@"slider3.png"] forState:UIControlStateNormal];
        
	UIImage *thumbImage = [UIImage imageNamed:@"slider2_resized.png"];
    ...
    NSString *labelText = NSLocalizedString(@"Slide to Unlock My Story", @"SlideToUnlock label"); // Slide to Unlock UILabel
	UIFont *labelFont = [UIFont fontWithName:@"HelveticaNeue-UltraLight" size:28];
	...
	CGFloat labelHorizontalCenter = slideToUnlock_Slider.center.x;
    ...
    if (!IS_IPHONE_5) { // iPhone 3G, 3GS, 4, 4S Support
...
- (id)initWithDelegate:(id<SlideToUnlockDelegate>)_delegate // Slide to Unlock Delegate
{    
    if (self = [self init]) {
...
- (void)viewDidLoad
{
    [self renderSlideToUnlockUIComponents]; // Rendering the UI Components
    [super viewDidLoad];
}
